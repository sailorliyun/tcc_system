...
Option Explicit
Public Const cpubMyPrjName = "QPAV_CEnt080" 'ƒvƒƒOƒ‰ƒ€–¼
...
Public strErrDescription      As String     'ƒGƒ‰[ƒƒbƒZ[ƒW
Public gblnExecute            As Boolean    'q‰æ–Ê‚ÅÀs‚³‚ê‚½‚©”»’è—p
Public gblnHaitaFlg           As Boolean    '”r‘¼”»’è—pƒtƒ‰ƒO
Public gstrSysDate            As String     'ƒT[ƒo[ƒVƒXƒeƒ€“ú•t
Public gblnNayoseFlg          As Boolean    '–¼Šñ‚¹ŒŸõ‰æ–Ê‹N“®ƒtƒ‰ƒO
...
Public gstrYuusiDat           As String     '—Z‘ƒf[ƒ^i‹æØ‚è•¶š“ü‚èj
...
Public gstrHensaiTukisu       As String     '•ÔÏŒ”
Public gstrKyuyoHensaiAll     As String     '‹‹—^•ÔÏ‘Šz
Public gstrKyuyoHensai        As String     '‹‹—^•ÔÏŠz
Public gstrSyoyoHensaiAll     As String     'Ü—^•ÔÏ‘Šz
Public gstrSyoyoHensai        As String     'Ü—^•ÔÏŠz
Public gstrKyuyoZandaka       As String     '‹‹—^c‚
Public gstrSyoyoZandaka       As String     'Ü—^c‚
Public gstrKojoChusiFlg       As String     'Tœ’†~ƒtƒ‰ƒO
Public gstrKansaiYMD          As String     'Š®Ï“ú

Public gstrSyoyoHensaigaku    As String     '‹‹—^ŒX•ÔÏŠz
Public gstrKyuyoHensaigaku    As String     'Ü—^•ÔÏŠz
...
On Error GoTo errgblnHaita
    ...
    odbDatabase.Parameters.Add "InstrSikibetu", "080", ORAPARM_INPUT                  '¯•ÊƒR[ƒh
    odbDatabase.Parameters.Add "InstrProgramID", cpubMyPrjName, ORAPARM_INPUT         'ƒvƒƒOƒ‰ƒ€‚h‚c
...
'ƒGƒ‰[ˆ—
    ...
    '”r‘¼ˆ—’†ƒGƒ‰[ƒƒbƒZ[ƒW•\¦
    If odbDatabase.LastServerErr >= 20000 Then   'Msg = ‘¼‚Ì’[––‚Åˆ—’†‚Å‚·BÄ“xÀs‚µ‚Ä‰º‚³‚¢
...
On Error GoTo asd
    ...
    intHensaiTuki = CInt(gstrHensaiTukisu)               '•ÔÏŒ”
    lngKyuuyoHensaiAll = CLng(gstrKyuyoHensaiAll)          '‹‹—^•ÔÏŠz(‘Šz)
    lngSyouyoHensaiAll = CLng(gstrSyoyoHensaiAll)         'Ü—^•ÔÏŠz(‘Šz)
...
''''    lngKyuuyoHensai = CLng(gfncstrDlm2(gstrYuusiDat, 9))             '–ˆŒ‹‹—^•ÔÏŠz
''''    lngSyouyoHensai = CLng(gfncstrDlm2(gstrYuusiDat, 11))            '–ˆŒÜ—^•ÔÏŠz
...
End Sub
'******************************************************************************
'*    •ÔÏ—\’è•\–¾×ì¬_V (—Z‘“ú‚ª2000/03/01ˆÈ~‚ÌŒvZ)
'******************************************************************************
...
''''    gstrBonusMD1 = Format$(Left$(gstrBonusMD1, 2) & Right$(gstrBonusMD1, 2), "MMDD")                  'Ü—^x•¥Œ“ú‚P
''''    gstrBonusMD2 = Format$(Left$(gstrBonusMD2, 2) & Right$(gstrBonusMD2, 2), "MMDD")                  'Ü—^x•¥Œ“ú2
    lngKyuuyoHensai = CLng(gstrKyuyoHensaigaku)             '–ˆŒ‹‹—^•ÔÏŠz
    lngSyouyoHensai = CLng(gstrSyoyoHensaigaku)            '–ˆŒÜ—^•ÔÏŠz
    
    '—Z‘“ú‚ª 2000/11/09 2000/11/10 ‚Ìê‡‚Í ’¼‹ß‚Ìƒ{[ƒiƒX“ú‚ğ‚UŒ‚P‚T‚É‚·‚é(’¼‹à‚Ì‚P‚QŒ‚P“ú‚Ìƒ{[ƒiƒX“ú‚ğ–³‹‚·‚é )
        ...
        For j = 1 To cnsMaxRec '
            ...
            With gstrYoteihyouAry(i)
                ...
                If Trim$(frmQPAV_CEnt081.txtYuusibiY.Text) & Trim$(frmQPAV_CEnt081.txtYuusibiM.Text) & Trim$(frmQPAV_CEnt081.txtYuusibiD.Text) < "20000301" Then
                    If Mid$(.strKijitu, 1, 2) > "11" Then
                        If Mid$(.strKijitu, 4, 5) = " 4.30" Then
                            OraPArray_04.Put_value Mid$(.strKijitu, 1, 2) & ". 6.15", (j - 1)                                            '04 •ÔÏŠú“ú
                        ElseIf Mid$(.strKijitu, 4, 5) = "10.31" Then
                            OraPArray_04.Put_value Mid$(.strKijitu, 1, 2) & ".12. 1", (j - 1)       '04 •ÔÏŠú“ú
                        Else
...
'MOD START 2019/05/07 ¼—ï2Œ…‚É•ÏX
