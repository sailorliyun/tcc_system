...
Private mdblKyuZanSum       As Double            '証書・給与残高計
Private mdblSyoZanSum       As Double            '証書・賞与残高計
...
Public Sub subShowErrorMsgBox(strErrCode As String, strErrMsg As String)
    ...
    Load QSAV_SysErr
    Call QSAV_SysErr.subSysErr("社員融資問合わせ", _
              gstrCommandLine(1), _
              gstrCommandLine(5), _
...
'    End If
    ...
    '画面に表示
    lblKyuyoHensai.Caption = Format$(odbDatabase.Parameters("OtNumKyuyoHensai").Value, "#,##0") '給与返済額
    lblSyoyoHensai.Caption = Format$(odbDatabase.Parameters("OtNumSyoyoHensai").Value, "#,##0") '賞与返済額
    lblZandaka.Caption = Format$(odbDatabase.Parameters("OtNumZanHensai").Value, "#,##0")       '残高返済額
...
On Error GoTo errGetSyouMeisaiInf
    ...
    Do
        ...
        '最終の設定
'        lngRecCount = lngRecCount + odbDatabase.Parameters("OtNumAryCount")        '【Win2000対応】Del by Yokoyama@NBC 2004/04/16
        lngRecCount = lngRecCount + CLng(odbDatabase.Parameters("OtNumAryCount"))   '【Win2000対応】Add by Yokoyama@NBC 2004/04/16
...
Private Sub subDispList()
 ...
 Dim dblSyoHen      As Double
    ...
    mdblKyuZanSum = 0           '証書・給与残高計
    mdblSyoZanSum = 0           '証書・賞与残高計
    ...
    With sprList
        ...
        'ｽﾌﾟﾚｯﾄﾞｼｰﾄのMaxRowを設定(max30をこえたら設定する
