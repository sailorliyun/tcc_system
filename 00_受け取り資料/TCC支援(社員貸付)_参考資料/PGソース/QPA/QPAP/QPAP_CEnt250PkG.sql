...
/* システム名    ：高島屋クレジットシステム                                  */
/* 業務名        ：社員融資管理                                              */
/* モジュール名  ：QPAP_CEnt250PkG                                           */
/* 機能          ：×控除連絡表印刷  ← ○控除データ作成                     */
/*                 毎月５日に起動されるCOBOLよりCALLされる                   */
/* 作成日        ：2001/08/25                                                */
/* 作成者        ：FIP                                                       */
/*****************************************************************************/
/* 修正履歴                                                                  */
/* 001：2005/08/29 ：NBC SEKINE                                              */
/*     障害対応                                                              */
/*       小口データの利息計算で利率が初期設定されない為、利息が計算されない  */
/*                                                                           */
/* 002: 2009/08/29 : NBC ISHIDA                                              */
/*     0811051_グループ社員証書貸付帳票見直し                                */
/*        完済日が入力されているデータは、全て控除データがされない為、       */
/*        完済日により控除データを作成する条件を追加                         */
/*****************************************************************************/
...
PROCEDURE QPAP_CEnt250KojoTKousin (
  ...
  InStrComputerNM     IN  VARCHAR2,                                   --コンピュータ名
  InStrYMD            IN  VARCHAR2,                                   --賞与日付
  InStrSyoriKbn       IN  CHAR,                                       --処理区分（ １…給与　２…賞与）
  OtStrPkgStatus      OUT CHAR);                                      --ステータス
...
/*****************************************************************************/
/* モジュール名  ：QPAP_CEnt250GetKoujyo       　　　　                      */
/* 機能          ：控除ＷＯＲＫテーブルを作成し、控除テーブルを更新する      */
/*****************************************************************************/
PROCEDURE QPAP_CEnt250GetKoujyo (
  InStrSyoriKbn       IN  VARCHAR2,                               --処理区分（融資区分）
  InStrSyoriYM        IN  VARCHAR2,                               --処理年月
  ...
  InStrComputerNM     IN  VARCHAR2,                               --コンピュータ名
  OtNumCount          OUT NUMBER,                                 --データ件数
  OtStrPkgStatus      OUT CHAR) IS                                --ステータス
  ...
  StrUserID       VARCHAR2(8);            --ユーザーＩＤ
  StrComputerNM   VARCHAR2(20);           --コンピュータ名
...
BEGIN
  ...
  gbl_ZenRiritu := QPAP_CEnt250GetRiritu(StrWorkYMD);     --前月利率
  ...
  gbl_ZenZenRiritu := QPAP_CEnt250GetRiritu(StrWorkYMD);  --前々月利率
...
/*****************************************************************************/
/* モジュール名  ：QPAP_CEnt250KeikaZandaka        　　                      */
/* 機能          ：控除連絡表_経過残高ＷＯＲＫ作成                           */
/*****************************************************************************/
PROCEDURE QPAP_CEnt250KeikaZandaka (
  InStrUserID     IN  VARCHAR2,                         --ユーザーＩＤ
...
/*****************************************************************************/
/* モジュール名  ：QPAP_CEnt250KogutiRisoku        　　                      */
...
--    COMMIT;
    ...
    LOOP
      ...
      END IF;
		...
		--******** FROM ******* 2005/08/29 障害対応 NBC SEKINE ***********
		v_StartRiritu := QPAP_CEnt250GetRiritu(v_YMD);				--利息計算開始日時点での利息率
		v_NextRiritu  := QPAP_CEnt250GetRiritu(v_ZenYMD);			--利息計算終了日時点での利息率
		--******** TO   ******* 2005/08/29 障害対応 NBC SEKINE ***********
		...
		IF v_KBN = '1' THEN
			...
			IF v_StartRiritu <> v_NextRiritu THEN
				...
				LOOP
					...
					v_GetRiritu  := QPAP_CEnt250GetRiritu(w_Cnt);						--日ごとに適用される利率(旧制度)を取得する
		...
		IF v_KBN = '2' THEN
			...
			v_StartRiritu := QPAP_CENT250GETRIRITU(v_YMD + 1);							--利息計算開始日時点での利息率
			v_NextRiritu  := QPAP_CENT250GETRIRITU(v_ZenYMD_N);							--利息計算終了日時点での利息率
			...
			IF v_StartRiritu <> v_NextRiritu THEN
				...
				LOOP
					...
					v_GetRiritu  := QPAP_CENT250GETRIRITU(w_Cnt);						--その日に適用される利率(旧制度)を取得する
					v_GetRiritu2 := TRUNC(v_GetRiritu / 36500 * 1000000) / 1000000;		--					利率(新制度)を取得する
		...
		IF v_KBN = '4' THEN
			...
			v_StartRiritu := QPAP_CENT250GETRIRITU(v_YMD + 1);							--利息計算開始日時点での利息率
			v_NextRiritu  := QPAP_CENT250GETRIRITU(InStrYMD);							--利息計算終了日時点での利息率
...
/*****************************************************************************/
/* モジュール名  ：QPAP_CEnt250KyuyoRisoku   　 　                           */
/* 機能          ：控除連絡表_利息ＷＯＲＫ作成                           　　*/
/*****************************************************************************/
PROCEDURE QPAP_CEnt250KyuyoRisoku (
  InStrUserID     IN  VARCHAR2,                         --ユーザーＩＤ
...
--2002/03/07 ここまで
      ...
      IF  v_KASIRISOKU IS NULL THEN
          v_KASIRISOKU := 0;
      END IF;
	v_StartRiritu := QPAP_CEnt250GetRiritu(v_ZenYMD);						--利息計算開始日時点での利息率
	v_NextRiritu  := QPAP_CEnt250GetRiritu(InStrYMD);							--利息計算終了日時点での利息率
		...
		ELSE
			IF v_StartRiritu <> v_NextRiritu THEN
				...
				LOOP
					...
					v_GetRiritu  := QPAP_CENT250GETRIRITU(w_Cnt);						--その日に適用される利率を取得する
					v_GetRiritu2 := TRUNC(v_GetRiritu / 36500 * 1000000) / 1000000;		--					利率を取得する
...
/*****************************************************************************/
/* モジュール名  ：QPAP_CEnt250KyuZanSyukei        　 　                     */
/* 機能          ：給与残高集計                                          　　*/
/*****************************************************************************/
PROCEDURE QPAP_CEnt250KyuZanSyukei (
  InStrUserID     IN  VARCHAR2,                         --ユーザーＩＤ
...
/*****************************************************************************/
/* モジュール名  ：QPAP_CEnt250KojoPrtKyuyo     　                           */
/* 機能          ：控除連絡表_印刷ＷＯＲＫ作成                           　　*/
/*****************************************************************************/
PROCEDURE QPAP_CEnt250KojoPrtKyuyo (
  InStrUserID     IN  VARCHAR2,                         --ユーザーＩＤ
...
BEGIN
    ...
    LOOP
      ...
      /* 2001/10/12 東西区分０ 不備対応  UPD Cent140と違う箇所*/
      ...
      /* 2001/10/12 東西区分名称 追加 UPD Cent140と違う箇所*/
      ...
      IF  v_OutPutFLG = 1   THEN    --On(出力する)
          ...
          /* 2001/10/12 集計単位・出力順 不備対応  UPD Cent140と違う箇所 */
          --SELECT  QSA2_MEI
          --  INTO  v_MEI
          --FROM    QSA2_SCODET
          ...
          INSERT INTO QPAO_KOJOW VALUES (
            ...
            ,NULL                         --行
            ,NVL(v_SEISANCD,' ')          --精算店コード    -- 10/12 UPD Cent140と違う箇所
            ,NVL(v_SEISANNM,' ')          --精算店名
            ...
            ,NVL(v_SYUBETU,' ')           --ローン種別
            ,NVL(v_TOZAIKBN,' ')          --東西区分
            ,NVL(v_MISECD,' ')            --店コード    -- 10/12 UPD Cent140と違う箇所
            ,NVL(v_MISENM,' ')            --店名
            ,NVL(v_BUMONNM,' ')           --部門名
            ,NVL(v_SIMEIKANJI,' ')        --氏名
            ,v_YUSIYMD                    --融資日
...
/*****************************************************************************/
/* モジュール名  ：QPAP_CEnt250SyouyoRisoku   　 　                          */
/* 機能          ：控除連絡表_賞与利息ＷＯＲＫ作成                         　*/
/*****************************************************************************/
PROCEDURE QPAP_CEnt250SyouyoRisoku (
  InStrUserID     IN  VARCHAR2,                         --ユーザーＩＤ
...
--2002/03/07 ここまで
      ...
      /* 2001/10/12 東西区分０ 不備対応 Cent140と違う箇所 */
      --IF  v_TOZAIKBN = '0' THEN     --東西区分 不定
      --    IF v_TOZAIKBNE = '1' THEN
      --       v_TOZAIKBN := '1';
      --    ELSE
    ...
    LOOP
      ...
      IF  v_BONUSMM1 = v_SYOYOMM    OR    v_BONUSMM2 = v_SYOYOMM  THEN
        ...
        IF  TO_CHAR(v_PRMYUSIYMD,'YYYYMMDD') <= gbl_SyousyoYMD  THEN
          ...
          /* 利息計算 */
          v_Risoku := QPAP_CEnt250SyoRisokuFnc(v_PRMYUSIYMD,        --融資日
...
/*****************************************************************************/
/* モジュール名  ：QPAP_CEnt250KojoPrtSyouyo                                 */
/* 機能          ：控除連絡表_賞与印刷ＷＯＲＫ作成                       　　*/
/*****************************************************************************/
PROCEDURE QPAP_CEnt250KojoPrtSyouyo (
  InStrUserID     IN  VARCHAR2,                         --ユーザーＩＤ
...
BEGIN
    ...
    LOOP
      ...
      /* 2001/10/12 東西区分名称 追加  Cent140と違う箇所 */
      --IF  v_TOZAIKBN = '1' THEN
      --    v_SEISANNM := RTRIM(v_SEISANNM) || ' (東)';
      --ELSE
      --    v_SEISANNM := RTRIM(v_SEISANNM) || ' (西)';
...
/*****************************************************************************/
/* モジュール名  ：QPAP_CEnt250SyoRisokuFnc   　 　                          */
...
/*****************************************************************************/
/* モジュール名  ：QPAP_CEnt250KojoTKousin                                   */
/* 機能          ：QPAO_KOJOWからデータを取得し、控除テーブル（QPAA_KOJOT）  */
/*                 の更新処理を行う                                          */
/*****************************************************************************/
PROCEDURE QPAP_CEnt250KojoTKousin (
  InStrUserID     IN  VARCHAR2,                         --ユーザーＩＤ
...
/*****************************************************************************/
/* モジュール名  ：QPAP_CEnt250ZanTKousin                                    */
/* 機能          ：QPAO_KOJOWからデータを取得し、残高テーブル（QPA0_ZandakaT）  */
/*                 の更新処理を行う                                          */
/*****************************************************************************/
PROCEDURE QPAP_CEnt250ZanTKousin (
  InStrUserID     IN  VARCHAR2,                         --ユーザーＩＤ
